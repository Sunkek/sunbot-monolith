...
CREATE TABLE IF NOT EXISTS guilds (
    ...
    rank_basic_member_role_verification boolean DEFAULT 'false',
    ...
    verification_message_text varchar(2000),
    verification_message_embed varchar(6000),
);
"""
CREATE_USERS_TABLE = """
...
def format_settings_value(guild, value):
    if type(value) == list:
        ...
        if int_convertable(value) and not type(value) == bool:
            ...
            str(value).startswith("{") and str(value).endswith("}") or \
            len(str(value)) > 200:
            result = "Set"
        elif value == True:
            result = "On"
